...
4. On the LLM model page that opens:
    ...
    3. If the provider is a third-party provider that requires a custom API key, first create the API key on the provider site, then copy and set the API key on your Hugging Face profile settings:
...
To create a chatflow in Flowise with a ChatHuggingFace chat model:
    ...
    3. Click **+** and drag **Chat Models** > **ChatHuggingFace**:
        * **Connect Credential**:
            Click **Create New** to create a new credential and enter the Hugging Face access token in the **HuggingFace API Key** field.
