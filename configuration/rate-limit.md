...
When you share your chatflow publicly with no API authorization through the API or embedded chat, anybody can access the flow. To prevent spamming, you can set the rate limit on your chatflow.
...
Using the example above, that means only 20 messages are allowed to be received in 60 seconds. The rate limitation is tracked by IP-address. If you have deployed Flowise on a cloud service, you'll have to set the `NUMBER_OF_PROXIES` env variable.
...
3. **Restart Cloud-Hosted Flowise Service:** This enables Flowise to apply changes for environment variables.
...
6. **Incorrect IP Address:** If the returned IP address does not match your current IP address, increase `NUMBER_OF_PROXIES` by 1 and restart the cloud-hosted Flowise. Repeat this process until the IP address matches your own.
